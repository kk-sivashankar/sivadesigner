...
function initGallery() {
  ...
  $galleryItems.on('click', e => {
    ...
    const $currentItem = $target.closest('.gallery-item');

    // Check if the click was on the link icon
    if ($target.hasClass('link-icon')) {
      const linkUrl = $currentItem.data('link');
      if (linkUrl) {
        window.open(linkUrl, '_blank');
      }
      return; // Prevent lightbox from opening
    }

    // Check if the click was on the zoom icon or the image itself
    if ($target.hasClass('zoom-icon') || $target.hasClass('thumb') || $target.hasClass('caption')) {
      const $currentGallery = $currentItem.closest('.gallery');
    ...
    // If the click was directly on the gallery item (and not on an icon or the image elements), also open the lightbox
